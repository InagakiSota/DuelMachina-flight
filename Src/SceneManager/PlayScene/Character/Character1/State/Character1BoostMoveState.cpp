...
void Character1BoostMoveState::Update()
{
	...
	//上入力
	if (keyState.IsKeyDown(DirectX::Keyboard::Keys::Up) && m_pCharacter->GetPos().y <  Character1Params::MOVE_LIMIT_TOP)
	{
		vel.y = 1;
		//アニメーションの切り替え
	...
	//下入力
	else if (keyState.IsKeyDown(DirectX::Keyboard::Keys::Down) && m_pCharacter->GetPos().y >  Character1Params::MOVE_LIMIT_BOTTOM)
	{
		vel.y = -1;
		//アニメーションの切り替え
		m_pCharacter->ChangeAnimation(static_cast<int>(eCHARACTER_ANIMATION_NUMBER::MOVE_BOOST));
	...
	m_pCharacter->SetVel(vel * Character1Params::GetInstance()->MOVE_BOOST_FORCE);
